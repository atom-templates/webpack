...
function execute(req, res, next, url, file) {
...
exports.cgi = ({docRoot, match = defaultMatch, index}) => (req, res, next) => {
    ...
    if (index && fs.existsSync(index)) {
        execute(req, res, next, url, index);
        return;
    }

    let phpScript = path.join(docRoot, url.pathname);

    if (fs.existsSync(phpScript)) {
        execute(req, res, next, url, phpScript);
        return;
    }
